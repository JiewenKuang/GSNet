...
from model.layers import GraphConvolution, MaxPool, BodyConvClothGraphConvolution, PhysicsConvolution
...
class GSNet(tf.keras.Model):
    def __init__(self, psd_dim, checkpoint=None, rest_pose=rest_pose, train_type='supervised', *args, **kwargs):
        ...
        self.chi_fc = tf.keras.Sequential([
            tf.keras.layers.Dense(3, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc0'),
            tf.keras.layers.Dense(6, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc1'),
            tf.keras.layers.Dense(12, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc2'),
            tf.keras.layers.Dense(24, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc3'),
            tf.keras.layers.Dense(12, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc4'),
            tf.keras.layers.Dense(6, activation=tf.keras.activations.relu, use_bias=True, name='chi_fc5'),
            tf.keras.layers.Dense(3, use_bias=True, name='chi_fc6')
        ])
        self.chi_fc.build(input_shape=(1, 3))
    ...
    def get_error_vertex_index(self, vertex, B, B_F):
        B = np.float32(B)
        # build body KDTree
        tree = cKDTree(B)
        _, idx = tree.query(vertex.numpy(), n_jobs=-1)
        # to nearest
        D = vertex - B[idx]
        # body normals
        B_vn = tfg.geometry.representation.mesh.normals.vertex_normals(B, B_F)
        # corresponding normals
        VN = tf.gather(B_vn, idx, axis=0)
        # dot product
        dot = tf.einsum('ab,ab->a', D, VN)
        _vmask = tf.cast(tf.math.less(dot, 0), tf.float32)
        return tf.tile(tf.reshape(_vmask, (-1, 1)), (1, 3))
    ...
    def call(self, T, L, P, S, G, fabric, tightness, indices, with_body=False):
        ...
        for i in range(1, len(indices)):
            ...
            _V = V[s:e]
            _g = G[i - 1]
            _g_p = Data.pre_gt_buffer[i - 1]
            _s = S[i - 1]
            _b = B1[i - 1]
            _b_p = PB1[i - 1]
            acceleration = _b - _b_p
            vertex_index = self.get_error_vertex_index(tf.stop_gradient(_V), _b, self.SMPL[0].faces)
            _b_p = np.float32(_b_p)
            # build body KDTree
            tree = cKDTree(_b_p)
            _, idx = tree.query(_V.numpy(), n_jobs=-1)
            acceleration = acceleration[idx]
            _g_p = _g_p + acceleration
            vertex_index_list.append(_g_p * vertex_index)
            vertex_index_list2.append(tf.abs(vertex_index - 1))
        vertex_index_list = tf.concat(vertex_index_list, axis=0)
        vertex_index_list2 = tf.concat(vertex_index_list2, axis=0)
        V_chi = V * vertex_index_list2 + vertex_index_list
        return V, V_chi, B1
