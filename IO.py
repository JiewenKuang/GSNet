...
def readPC2(file, float16=False):
    assert file.endswith('.pc2') and not float16 or file.endswith(
        '.pc16') and float16, 'File format not consistent with specified input format'
    data = {}
...
def writePC2Frames(file, V, float16=False):
    ...
    if os.path.isfile(file):
        ...
        with open(file, 'rb+') as f:
            ...
            f.seek(16)
            nPoints = unpack('<i', f.read(4))[0]
            assert len(V.shape) == 3 and V.shape[1] == nPoints, 'Inconsistent dimensions: ' + str(
                V.shape) + ' and should be (-1,' + str(nPoints) + ',3)'
            # Read n. of samples
            f.seek(28)
            nSamples = unpack('<i', f.read(4))[0]
            # Update n. of samples
